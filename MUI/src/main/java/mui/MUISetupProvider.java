...
public class MUISetupProvider extends ComponentProviderAdapter {
	...
	private JTextField createPathInput(String defaultStr) {
		...
		JPanel inputRow = new JPanel(new RelativeLayout(RelativeLayout.X_AXIS));
		inputRow.setMinimumSize(new Dimension(800, 100));

		entry.setText(defaultStr);
		inputRow.add(entry, new Float(2));
		...
		inputRow.add(selectButton, new Float(1));
	...
	// RelativeLayout implementation from http://www.camick.com/java/source/RelativeLayout.java
	public class RelativeLayout implements LayoutManager2, java.io.Serializable {
		...
		private void layoutContainerHorizontally(Container parent) {
			...
			int spaceAvailable = parent.getSize().width - insets.left - insets.right -
				((visibleComponents - 1) * gap) - (2 * borderGap);
		...
		private void layoutContainerVertically(Container parent) {
			...
			float relativeTotal = 0.0f;
			Insets insets = parent.getInsets();
			int spaceAvailable = parent.getSize().height - insets.top - insets.bottom -
				((visibleComponents - 1) * gap) - (2 * borderGap);
		 ...
		 */
		private int[] allocateRelativeSpace(Container parent, int spaceAvailable,
				float relativeTotal) {
			int spaceUsed = 0;
			int components = parent.getComponentCount();
			...
			for (int i = 0; i < components; i++) {
				...
				if (relativeTotal > 0 && spaceAvailable > 0) {
					...
					if (constraint != null) {
						int space = (int) (Math
								.round(spaceAvailable * constraint.floatValue() / relativeTotal));
